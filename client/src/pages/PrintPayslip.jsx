...
const PrintPayslip = () => {
  // {} on id means i am destructuring the object given by useParams(). I get string from that!!😁
  const {id} = useParams()
  // console.log(id)
        ...
        <table className='w-full text-sm'>
          <thead>
            <tr className='bg-slate-50'>
              <th className='text-left py-3 px-4 text-sm text-slate-500 uppercase tracking-wider'>Description</th>
              <th className='text-right py-3 px-4 text-sm text-slate-500 uppercase tracking-wider'>Amount</th>              
            </tr>
          </thead>
          <tbody>
            <tr className='border-t border-slate-100'>
              <td className='py-3 px-4 text-slate-700'>Basic Salary</td>
              <td className='text-right px-4 py-3 text-slate-900 font-medium'>${payslip.basicSalary?.toLocaleString()}</td>
            </tr>
            <tr className='border-t border-slate-100'>
              <td className='py-3 px-4 text-slate-700'>Allowances</td>
              <td className='text-right px-4 py-3 text-slate-900 font-medium'>+${payslip.allowances?.toLocaleString()}</td>
            </tr>
            <tr className='border-t border-slate-100'>
              <td className='py-3 px-4 text-slate-700'>Deductions</td>
              <td className='text-right px-4 py-3 text-slate-900 font-medium'>-${payslip.deductions?.toLocaleString()}</td>
            </tr>
            <tr className='border-t-2 border-slate-200 bg-slate-50'>
              <td className='py-4 px-4 text-slate-900 font-bold'>Net Salary</td>
              <td className='text-right px-4 py-4 text-slate-900 font-bold text-lg'>${payslip.netSalary?.toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
